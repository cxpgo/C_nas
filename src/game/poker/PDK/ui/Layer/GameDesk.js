 ...
 */
var PDKGameDesk = cc.Layer.extend({
    panel_up:null,
    panel_left:null,
    panel_player:null,
    panel_right:null,
    ...
    ctor: function () {
        this._super();
        var root = util.LoadUI(PDKPokerJson.PDKDesk).node;
        this.addChild(root);
    ...
    onPlayerEnter: function (data) {
        ...
        if (MajhongInfo.GameMode == GameMode.PLAY) {
            if (XYGLogic.Instance.selfSeatInfo()) {
                ...
                if(!playerSeat){
                    playerSeat = new PaoSelfSeat(data);
            ...
            if (XYGLogic.Instance.leftSeatInfo()) {
                ...
                var playerSeat = this.playerSeatAll[data.uid];
                if(!playerSeat){
                    playerSeat = new PaoLeftSeat(data);
            ...
            if (XYGLogic.Instance.rightSeatInfo()) {
                ...
                var playerSeat = this.playerSeatAll[data.uid];
                if(!playerSeat){
                    playerSeat = new PaoRightSeat(data);
