var DDZGoldGameScene = cc.Layer.extend({
    _id: 0,
    text_round: null,
    ...
    ctor: function () {
        ...
        var root = util.LoadUI(DDZPokerJson.GameRoomGold).node;
        this.addChild(root);
        ...
        var enableShow = true;    // todo 是否显示电量和网络信号
        if (enableShow) {
    ...
    goldReadyEvent: function (data) {
        if(data.code == 400)
        {
            var dialog = new JJConfirmDialog();
            dialog.setDes("您的金币超过上限，推荐您去更高的场次！");
            dialog.showDialog();
        ...
        if (data) {
            ...
            if (this.checkMsg(data)) {
                ...
                for (var i = 0; i < infoPlayers.length; i++) {
                    ...
                    if (uid == hall.user.uid && ready != 1) {
                        XYGLogic.Instance.ready(this.goldReadyEvent.bind(this));
                    }
                }
            }
    ...
    onPlayerLeave: function (data) {
        ...
        if (XYGLogic.Instance.currentRound > 1)
            return;
        if (data.uid == hall.user.uid && this.ready_change == true) return;

        this.mGameDesk.onPlayerLeave(data);

        if (XYGLogic.Instance.status <= TABLESTATUS.READY && data['uid'] == hall.user.uid && XYGLogic.Instance.isRePrivateTable == 1) {
    ...
    onSelfLeave: function (status) {
        this.ready_change = status;
    }
});