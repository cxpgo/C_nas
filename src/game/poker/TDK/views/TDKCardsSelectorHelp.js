...
var TDKCardsSelectorHelp = cc.Layer.extend({
    panel_players:null,
    ...
    {
        this._super();
        var root = util.LoadUI(TDKJson.CardsSelector).node;
        this.addChild(root);
    ...
    onConfirm:function () {
        ...
        // TDKPoker.net.GMOp(hall.user.uid, laizi, function (data) {
    ...
    onAddCard:function (sender) {
        ...
        if(this.player_select > -1 && (this.cardsCount[sender.name] > 0 || this.player_select == 4))
        {
            ...
            //     return;
            if(this.player_cards[this.player_select].length >= 5)
                return;
            if (this.player_select == 5) {
