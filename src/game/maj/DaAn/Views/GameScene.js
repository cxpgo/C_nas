...
MJDaAn.GameScene = function () {
	var _GameStatus = MJDaAnGameStatus;
	var _GameDesk = MJGameDesk;
	// var _EndResult = MJDaAn.EndResult;
	// var _RoundResult = MJDaAn.RoundResult;
	var _RecordControll = MJDaAn.RecordControll;
	...
	var GameRoom = MJGameRoomBase.extend({
		setOption: function () {
			...
			this.UIPlugIns = [
				...
				{
					V2D: MJDaAn.RES.WGRoomBaoPai,
					V3D: MJDaAn.RES.WGRoomBaoPai3D,
				},
			];
		},
		ctor: function () {
			...
			MJDaHuAni.register(MJDaAn.DahuAnim);
            MJGameDesk.registCfg(MJDaAn.DeskCfg);
			var root = this.mRoot;
			var _this = this;
			...
			this.image_baopai = ccui.helper.seekWidgetByName(root, "image_baopai");
		...
		onEnter: function () {
			this._super();
			MJSoundMgr.RegistMJAudioPath(MJDaAn.RESPath+"Resoures/audio/");
		...
		onPlayerLeave: function (data) {
			var ok = this._super(data);
			if (ok) return;

			if (XYGLogic.table.status < _GameStatus.GAMERESULT && data['uid'] == data['fangZhu'] && XYGLogic.table.isRePrivateTable != 1) {
			...
			if (XYGLogic.table.status < _GameStatus.GAMERESULT && data['uid'] == hall.user.uid && XYGLogic.table.isRePrivateTable == 1) {
				XYGLogic.net.imRoomId = -1;
				if (cc.sys.isNative) {
					GameLink.onUserLeaveRoom();
				}
		...
		addShowBaoPai: function (cardObj) {
			this.image_baopai.removeAllChildren();
			if(cardObj){
			...
			// card.setScale(0.78);
			this.image_baopai.addChild(card);
			card.x = 10;
			card.y = 25;
			this.image_baopai.setVisible(true);
		},
			...
			if (!!data.pai && data.pai != undefined) {
				if (!!data['pai']['type'] && data['pai']['type'] != undefined) {
					...
					if (this.baoPaiKey == "nil") {
						...
						this.panel_infomation.addChild(sp_ani1, 100);
						var size = this.panel_infomation.getContentSize();
		...
		reset: function () {
			this._super();
			this.baoPaiKey = null;
			this.image_baopai.setVisible(false);
			this.image_baopai.removeAllChildren();
			this.image_baopai.setVisible(false);
