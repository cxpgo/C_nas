var MJUpSeatRe = function () {
    var Seat = MJUpSeat.extend({
        ...
        playRecordMoCard: function (data) {
            ...
            var card = MJCardUpShow.create3D(data, panelLength * 3 + length, CARD_SITE.RECORD);
        ...
        playRecordSendAction: function (card) {
            ...
            outCardRight.runAction(cc.sequence(delay, cc.callFunc(function () {
                sound.playCardDown();
            }),
        ...
        runRecordBuhuaOpAction: function (data) {
            ...
            for (var i = 0; i < huapai.length; i++) {
                var card = MJCardUpShow.create3D(huapai[i], panelLength * 3 + i + this.cardInArray.length - 1, CARD_SITE.RECORD);
                this.cardInArray.push(card);
        ...
        initRecordHandCards: function () {
            ...
            for (var p in cards) {
                ...
                for (var i = 0; i < arr.length; i++) {
                    var card = MJCardUpShow.create3D(arr[i], index, CARD_SITE.RECORD);
