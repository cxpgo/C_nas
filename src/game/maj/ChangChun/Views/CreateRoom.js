MJChangChun.WGCreateRoom = ccui.Layout.extend({
    mCacheTag : "config_changchun_",
    ctor: function () {
        var root = util.LoadUI(MJChangChun.RES.WGCRoom).node;
        this._super(root);

        this.mRoomData = {};
        this.mRoomData['uid'] = hall.user.uid;
        this.mRoomData['aaGem'] = 0;
        this.mRoomData['jiDan'] = 0;
        this.mRoomData['person'] = 4;
        this.mRoomData['sanFengDan'] = 0;
        this.mRoomData['danZhanLi'] = 0;
        this.mRoomData['rounds'] = 4;
        this.mRoomData['anBao'] = 0;
        this.mRoomData['queMen'] = 0;
        this.mRoomData['menQing'] = 0;
        this.mRoomData['daHuQue'] = 0;
        this.mRoomData['paoOne'] = 0; 
        this.mRoomData['cgfb'] = 0; 
        this.mRoomData['tableName'] = 'changchun';
        this.mRoomData['serverType'] = 'changchun';
    ...
    optView: function () {
        ...
        var anBaoCfgVs = [0 , 1];
        for (var i = 0; i < 2; i++) {
            var panel = ccui.helper.seekWidgetByName(this.panel_wanfa, "panel_wanfa1" + i);
            var checkbox = ccui.helper.seekWidgetByName(this.panel_wanfa, "checkbox_wanfa1" + i);
            checkbox.setTouchEnabled(false);
            panel._checkBox = checkbox;
            var saveOp = util.getCacheItem(this.mCacheTag + 'anBao');

            if (saveOp === "0" || saveOp === "1") {
                this.mRoomData["anBao"] = parseInt(saveOp);
            }
            var bl = this.mRoomData["anBao"] === anBaoCfgVs[i];
            checkbox.setSelected(bl);
            panel.setTouchEnabled(!bl);
            ...
            var clickData = {};
            clickData['this'] = this;
            clickData['itemValue'] = anBaoCfgVs[i];
            clickData['index'] = i;
            clickData['array'] = panel_wanFa1;
            clickData['itemKey'] = "anBao";
            panel.addClickEventListener(this.onToggle.bind(clickData));
        }
    },
    ...
    optSelectsWanFaView: function () {
        var opKeys = ["paoOne", "jiDan", "sanFengDan", "danZhanLi" , "queMen" , "daHuQue" , "cgfb"];
    ...
    getCreateRoomData:function () {
        var roomData = {};
        roomData['appId'] = MJChangChun.appId;
